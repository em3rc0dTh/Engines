...
import { ChannelAdapterRegistry } from '../channel-core/adapter-registry.js';
import { WebChatAdapter } from './webchat.adapter.js';

const CTA_BASE_URL = (process.env.ENGINES_CTA_URL ?? 'http://127.0.0.1:8787').replace(/\/$/, '');
const CHANNEL_BASE_URL = (process.env.ENGINES_CHANNEL_URL ?? 'http://127.0.0.1:8788').replace(/\/$/, '');
const TRUSTED_BUSINESS_SLUG = process.env.ENGINES_WEBCHAT_BUSINESS_SLUG?.trim() || 'golden-business';
...
function conversationIdFromPath(pathname: string): string | undefined {
  const prefix = '/api/channel/conversations/';
  const suffix = '/view';
  if (!pathname.startsWith(prefix) || !pathname.endsWith(suffix)) return undefined;
  const encoded = pathname.slice(prefix.length, -suffix.length);
  if (!encoded || encoded.includes('/')) return undefined;
...
async function workflowView(response: ServerResponse, workflowId: string): Promise<void> {
  ...
  sendJson(response, 200, { ...body, view: projectAppointmentWorkflow(state) });
}

async function conversationView(response: ServerResponse, externalConversationId: string): Promise<void> {
  const upstream = await fetch(
    `${CHANNEL_BASE_URL}/channel/conversations/${encodeURIComponent(externalConversationId)}?businessSlug=${encodeURIComponent(TRUSTED_BUSINESS_SLUG)}&channel=WEBCHAT`,
  );
  const payload = Buffer.from(await upstream.arrayBuffer());
  sendBuffer(response, upstream.status, upstream.headers.get('content-type') ?? 'application/json; charset=utf-8', payload);
}
...
async function channelEvent(request: IncomingMessage, response: ServerResponse, registry: ChannelAdapterRegistry): Promise<void> {
  ...
  const envelope = adapter.normalizeInbound(raw, { businessSlug: TRUSTED_BUSINESS_SLUG });
  const upstream = await fetch(`${CHANNEL_BASE_URL}/channel/events`, {
    method: 'POST',
  ...
  const server = createServer(async (request, response) => {
    try {
      ...
      if (request.method === 'GET') {
        const externalConversationId = conversationIdFromPath(url.pathname);
        if (externalConversationId) {
          await conversationView(response, externalConversationId);
          return;
        }
      ...
      const status = message.startsWith('CHANNEL_') ? 400 : 500;
      sendJson(response, status, { ok: false, code: status === 400 ? message.split(':')[0] : 'WEBCHAT_INTERNAL_ERROR', error: message });
    }
  });

  server.listen(PORT, HOST, () => {
    console.log(`ENGINES_WEBCHAT_C1B_READY http://${HOST}:${PORT}/webchat/ -> ${CHANNEL_BASE_URL}`);
  });
}

run().catch((error: unknown) => {
  console.error(`ENGINES_WEBCHAT_C1B_FAILED ${error instanceof Error ? error.stack ?? error.message : String(error)}`);
  process.exitCode = 1;
});
