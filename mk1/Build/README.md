...
The final certification authority is run `33538554471`, which includes that safety proof.

Intermediate S4 runs cancelled by `cancel-in-progress` while newer commits were pushed are classified as superseded executions, not product/test failures.
...
## S4 evidence highlights

The final S4 artifact contains:

- source SHA captured from the actual checkout;
- Compose state/logs;
- PostgreSQL `service_mutation_commands` ledger;
- Mongo `services_mutation_audit` terminal evidence.

The executed scenario proves exact replay, material conflict, business-scoped idempotency, Service/Offering revision progression, stale-revision rejection, lifecycle state changes and missing-dependency no-partial-effect safety.
...
S5 must prove a running Temporal consumer retains a selected Service/Offering revision snapshot while S4 publishes a newer catalog revision, and that a new Workflow sees the newer revision.

## Cross-reference

- Gate plan: [`../Plan/01-services-engine-gates.md`](../Plan/01-services-engine-gates.md)
- Consolidated test ledger: [`../Test/g1-services-engine-s0-s4.md`](../Test/g1-services-engine-s0-s4.md)
...
- Parallel CTA design: [`../Design/02-cta-channel-adapter-contract.md`](../Design/02-cta-channel-adapter-contract.md)
